...
class CodeToGraph:
    ...
    def parse_code(self, code: str):  # sourcery skip: for-index-underscore
        tree = ast.parse(code)
        for node in ast.walk(tree):
            if isinstance(node, ast.Assign):
                for target in node.targets:
                    if isinstance(node.value, ast.Num):
                        self.nodes.append(NumberNode(node.value.n, self))
                    elif isinstance(node.value, ast.BinOp) and isinstance(node.value.op, (ast.Add, ast.Mult)):
                        if isinstance(node.value.op, ast.Add):
                            self.nodes.append(SumNode(self))
                        elif isinstance(node.value.op, ast.Mult):
                            self.nodes.append(ProductNode(self))
                    elif isinstance(node.value, ast.Call) and node.value.func.id == 'print':
                        self.nodes.append(CallNode("print", self))
            elif isinstance(node, ast.FunctionDef):
                self.nodes.append(FunctionNode(node.name, self))
            elif isinstance(node, ast.Return):
                #self.nodes.append(ReturnNode(self))
                pass
